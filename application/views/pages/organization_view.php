<?php 
  ...
  $module_id = 0000;
  $module_data = $this->db->get_where("application_modules",array("M_Name"=>"Company Profile","Deleted"=>0,"Status"=>1));
  ...
  $this->db->where(array("organizations.Deleted"=>0,"organizations.Id"=>$this->org_id));
  $this->db->select("
                      organizations.*, 
    ...
    if($organizations->num_rows() > 0)
    {
      ...
      $get_subscription_rec = $this->db->get_where("organization_subscriptions",array("Org_Id"=>$this->org_id,"Subscription_Status"=>1));
  ...
  <div class="contentpanel">
    <div class="panel">
      <div class="panel-body">
        <div class="col-md-12">
          <div class="col-md-6">
            <h3><i class="fa fa-building-o"></i> Organization Information  </h3>
          </div>
          <div class="col-md-6"> 
            <?php if(in_array($module_id."_edit", $this->role_permissions)){ ?>
            <button onclick="load_view(this,'form_organization','<?= $this->org_id; ?>')" style="float: right;" class="btn btn-primary btn-quirk"><i class="fa fa-pencil"></i>&nbsp; Edit Information </button> 
          <?php } ?>
          </div>
        </div>
      </div>
