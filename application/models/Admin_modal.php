...
class Admin_modal extends CI_Model {
   ...
   //  echo "</pre>";   
      ...
      $this->db->select('
                    applicants.*, 
                                ...
                                ')
          ->from('applicants')
          ->join("applicant_applications","applicant_applications.Applicant_Id = applicants.Id AND applicant_applications.Org_Id = $this->org_id","left")
          ...
          $where = array(
            ...
            if(isset($filter_data['Job_Id']) && $filter_data['Job_Id'] > 0) { $where['applicant_applications.Job_Id'] = $filter_data['Job_Id']; }
            if(isset($filter_data['Location_Id']) && $filter_data['Location_Id'] > 0) { $where['locations.Id'] = $filter_data['Location_Id']; }
            if(isset($filter_data['Nationality']) && $filter_data['Nationality'] > 0) { $where['applicants.Nationality'] = $filter_data['Nationality']; }

            if(isset($filter_data['Application_Status']) && $filter_data['Application_Status'] != '0') { $where['applicant_applications.Application_Status'] = $filter_data['Application_Status']; }

            if(isset($filter_data['Location_Id']) && $filter_data['Location_Id'] > 0) { $where['applicant_applications.Location_Id'] = $filter_data['Location_Id']; }

            if(isset($filter_data['Job_Id']) && $filter_data['Job_Id'] > 0) { $where['applicant_applications.Job_Id'] = $filter_data['Job_Id']; }

            if(isset($filter_data['Country']) && $filter_data['Country'] > 0) { $where['applicants.Country'] = $filter_data['Country']; }
            if(isset($filter_data['State']) && $filter_data['State'] > 0) { $where['applicants.State'] = $filter_data['State']; }
            if(isset($filter_data['City']) && $filter_data['City'] > 0) { $where['applicants.City'] = $filter_data['City']; }
            if(isset($filter_data['Gender']) && $filter_data['Gender'] != '0') { $where['applicants.Gender'] = $filter_data['Gender']; }
            if(isset($filter_data['From_Apply_Date']) && $filter_data['From_Apply_Date'] != ""){ 
              $where['applicant_applications.Date_Added >='] = date("Y-m-d H:i:s",strtotime($filter_data['From_Apply_Date']));  
            }
            if(isset($filter_data['To_Apply_Date']) && $filter_data['To_Apply_Date'] != ""){ 
              $where['applicant_applications.Date_Added <='] = date("Y-m-d 23:59:59",strtotime($filter_data['To_Apply_Date']));  
            }
            if(isset($filter_data['Degree_Type']) && $filter_data['Degree_Type'] != '0'){ 
               ...
               $this->db->join("applicant_education", "applicant_education.Id = (select max(Id) from applicant_education as e2 where e2.Applicant_Id = applicants.Id AND e2.Degree_Type='$filter_data[Degree_Type]' )", 'left');
               $where['applicant_education.Degree_Type'] = $filter_data['Degree_Type'];  
                 ...
                 $this->db->join('applicant_education', 'applicant_education.Id = (select max(Id) from applicant_education as e2 where e2.Applicant_Id = applicants.Id AND (e2.Degree_Type=\'Bachelors 4 Years\' OR e2.Degree_Type=\'MS\'))', 'left');
            }
            if(isset($filter_data['Institute']) && sizeof($filter_data['Institute']) > 0) {

              if( $filter_data['Institute'][0]  > 0){ 
                $this->db->where_in("applicant_education.Institute",$filter_data['Institute']); 
              } 
            }
            if(isset($filter_data['Marks_Obtained']) && $filter_data['Marks_Obtained'] > 0) {

              if(isset($filter_data['Marks_Value']))
              {
      ...
      if(isset($filter_data['Applicant_Type']) && $filter_data['Applicant_Type'] != '0')
      { 
        if($filter_data['Applicant_Type'] == "New")
          $this->db->having('applicant_total_applications <= 1'); 
        elseif($filter_data['Applicant_Type'] == "Returning")
          $this->db->having('applicant_total_applications >= 2');
        elseif($filter_data['Applicant_Type'] == "Not Applied")
          $this->db->having('applicant_total_applications <= 0'); 
      } 

      if(isset($filter_data['Text']) && $filter_data['Text'] != "")
      {
        $like_query = array();
        $text_query = explode(",", $filter_data['Text']);

        if(sizeof($text_query) > 0){
          $this->db->group_start();
          
          for ($j=0; $j< sizeof($text_query); $j++){
            $text_query[$j] = trim($text_query[$j]);
            $like_query["applicants.First_Name"] = $text_query[$j];
            $like_query["applicants.Last_Name"] = $text_query[$j];
            $like_query["applicants.Email"] = $text_query[$j];
            $like_query["applicants.Phone_Number"] = $text_query[$j];
            $like_query["applicants.Mobile_Number_1"] = $text_query[$j];
            $like_query["applicants.Mobile_Number_2"] = $text_query[$j];
            $like_query["applicants.Gender"] = $text_query[$j];
            $like_query["applicants.CNIC"] = $text_query[$j];
            $like_query["applicants.Religion"] = $text_query[$j];
            $like_query["applicants.Blood_Group"] = $text_query[$j];
            $like_query["applicants.Source_Of_Hire"] = $text_query[$j];

            $this->db->or_like($like_query);
  ...
  { 
      ...
      $this->db->join("applicant_education","applicant_education.Applicant_Id = applicants.Id  AND applicant_education.Degree_Type='Bachelors 4 Years'","left");
      $this->db->join("universities","universities.Id = applicant_education.Institute","left");
