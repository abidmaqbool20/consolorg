						...
						if($get_org_subscription->num_rows() > 0)
						{
							...
							if($get_org_plan->num_rows() > 0)
							{
								...
								elseif(strtotime($today) >= strtotime($subscription_start_date) && strtotime($today) <= strtotime($plan_end_date))
								{
									$attendance = "abscent";
									$attendance_id = 0;
									$attendance_rec = $this->db->get_where("attendance",array("Deleted"=>0,"Status"=>1,"Employee_Id"=>$user_info['Id'],"Org_Id"=>$user_info['Org_Id'],"Signin !="=>"0000-00-00 00:00:00","Signout"=>"0000-00-00 00:00:00"));
									if($attendance_rec->num_rows() > 0)
									{
										$attendance_data = $attendance_rec->result_array();
										$attendance = "present";
