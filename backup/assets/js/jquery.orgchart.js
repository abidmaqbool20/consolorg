(function($) {
    ...
    function OrgChart($container, opts){
        ...
        this.startEdit = function(id){
            ...
            inputElement.keyup(function(event){
                if(event.which == 13){
                    ...
                    $root_url = location.protocol + '//' + location.host+"/admin/update_organogram_child";
                    
                    nodes[id].data.name = inputElement.val();
                    $.ajax({
                                type: "POST",
                                url: $root_url,  
                                data: { Id:id,Name:inputElement.val() },
                                success: function (data) { },
                                error: function () { }
                            });

                    //$.post($root_url,{Id:id,Name:inputElement.val()},function(id){   });
                }
            });
        ...
        this.newNode = function(parentId,this_id){
            var nextId = Object.keys(nodes).length;
            while(nextId in nodes){
                nextId++;
            }

            self.addNode({id: this_id, name: '', parent: parentId});
        }
